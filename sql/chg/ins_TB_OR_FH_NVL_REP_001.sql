/*==============================================================================
* 프로그램ID      : 
* 프로그램명      : ins_TB_OR_FH_NVL_REP_001
 ...
 BEGIN 
  ...
  DBMS_OUTPUT.PUT_LINE('P_RUNDATE :기준일 : '||P_RUNDATE);
    ...
    (
       ...
       ,P_RKI_ID
       ,(
        SELECT AVG(DECODE(SUBSTR(A.BAS_YM,5,2),'03',0,'09',0,A.KRI_NVL-B.KRI_NVL))/DECODE(AVG(B.KRI_NVL),0,1,NULL,1,AVG(B.KRI_NVL))*100
          FROM OPEOWN.TB_OR_KH_NVL A
          LEFT JOIN OPEOWN.TB_OR_KH_NVL B
            ON A.OPRK_RKI_ID = B.OPRK_RKI_ID
           AND B.BAS_YM = TO_CHAR(ADD_MONTHS(TO_DATE(P_BASYM,'YYYYMM'),-6),'YYYYMM') 
         ...
         WHERE A.BAS_YM = TO_CHAR(TO_DATE(P_BASYM,'YYYYMM'),'YYYYMM')
