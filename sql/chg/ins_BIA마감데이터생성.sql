...
BEGIN
  ...
  SELECT NVL(A.BIA_EVL_ED_DT,B.DUMMY) BIA_EVL_ED_DT
        ,A.BAS_YM
        ,B.DUMMY
    INTO P_RUNDATE
        ,P_BASYM
  ...
  DBMS_OUTPUT.PUT_LINE('P_BF_BASYM :기준년월(이전평가) : '||P_BF_BASYM); 
          ...
          IF P_DTDATE = P_RUNDATE  THEN
                ...
                DBMS_OUTPUT.PUT_LINE('기준일 일치 평가데이터 생성을 시작합니다.');
                 ...
                 UPDATE OPEOWN.TB_OR_OM_SCHD_PLAN SET BIA_EVL_PRG_STSC ='03'
                ...
                UPDATE OPEOWN.TB_OR_OM_SCHD SET BIA_EVL_PRG_STSC ='03'
                WHERE BAS_YM = P_BASYM 
                ;
                P_LD_CN := SQL%ROWCOUNT;
   ...
   USING (
   SELECT BAS_YM,B.BIA_EVL_TGT_YN ,B.BIA_EVL_ST_DT,B.BIA_EVL_ED_DT,B.BIA_EVL_PRG_STSC
   ...
   WHERE BAS_YM = 
     ...
     WHEN MATCHED THEN UPDATE SET 
                  A.BIA_YY = SUBSTR(A.BAS_YM,0,4)
                 ,A.BIA_EVL_ST_DT = B.BIA_EVL_ST_DT
                 ,A.BIA_EVL_ED_DT = B.BIA_EVL_ED_DT
                 ,A.BIA_EVL_PRG_STSC = B.BIA_EVL_PRG_STSC
                 ,A.BIA_EVL_TGT_YN = B.BIA_EVL_TGT_YN
                 ,A.LSCHG_DTM = SYSDATE
                 ,A.LS_WKR_ENO = 'SYSTEM'
     WHEN NOT MATCHED THEN 
     INSERT (A.GRP_ORG_C,A.BAS_YM
                 ,A.BIA_EVL_ST_DT,A.BIA_EVL_ED_DT
                 ,A.BIA_EVL_PRG_STSC,A.BIA_EVL_TGT_YN,A.FIR_INP_DTM,A.FIR_INPMN_ENO,A.LSCHG_DTM,A.LS_WKR_ENO
                 )
           VALUES ('01',B.BAS_YM,B.BIA_EVL_ST_DT,B.BIA_EVL_ED_DT
                 ,B.BIA_EVL_PRG_STSC,B.BIA_EVL_TGT_YN,SYSDATE,'SYSTEM',SYSDATE,'SYSTEM'
                 )
             ;
             
           DBMS_OUTPUT.PUT_LINE(P_LD_CN || '다음BIA 스케쥴 생성 평가완료로 업데이트');
          ...
          WHERE BIA_EVL_TGT_YN = 'Y'      
                ...
                DBMS_OUTPUT.PUT_LINE(P_LD_CN || '다음 BIA 평가대상생성 ');
                COMMIT;
