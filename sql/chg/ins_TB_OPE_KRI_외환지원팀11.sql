/*
  프로그램명 : ins_TB_OPE_KRI_외환지원팀11
  타켓테이블 : TB_OPE_KRI_외환지원팀11
  KRI 지표명 : 미결제외환 미정리 건수(단기)
  협      조 : 고찬식차장
  최조작성자 : 최상원
  KRI 지표명 :
     - 외환지원팀-11 미결제외환 미정리 건수(단기)
       A: 전월 말 미결제 외환 중 취결당일로부터 7일 초과 건수
*/
DECLARE
...
BEGIN
  ...
  IF P_EOTM_DT = P_BASEDAY  THEN
    ...
    DELETE OPEOWN.TB_OPE_KRI_외환지원팀11
    WHERE  STD_DT = P_BASEDAY;

    DBMS_OUTPUT.PUT_LINE(SQL%ROWCOUNT || ' rows deleted');

    INSERT INTO OPEOWN.TB_OPE_KRI_외환지원팀11
    ...
    WHERE    1=1
    AND      A.NDFY_NSTL_DSCD  = '2'     -- 미지급미결제구분코드
    AND      A.TR_STCD = '1'
    AND      (
               -- 미정리건
    ...
    SP_INS_ETLLOG('TB_OPE_KRI_외환지원팀11',P_BASEDAY,P_LD_CN,'KRI_ETL');
  END IF;
