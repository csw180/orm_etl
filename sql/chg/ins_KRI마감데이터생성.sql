...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('P_BF_BF_BASYM :기준년월전전월: '||P_BF_BF_BASYM); 
         ...
         SELECT                                                                 
			 ...
			 AND B.KRI_EVL_TGT_YN = 'Y'
                ...
                UPDATE  OPEOWN.TB_OR_KS_RKI_BR_SMST
                  SET KRI_GRDNM = 'GREEN'
                WHERE BAS_YM IN ('202204','202205','202206','202207','202208','202209','202210','202211','202212','202301','202302','202303','202304','202305','202306')
                 AND KRI_LMT_DSC IN ('02','03')
                 ;
                 /* 운영리스크 오픈 6개월간 데이터 수집을 위하여 이동평균 , 전년평균 보류 */
   ...
   IF P_DTDATE = P_RUNDATE  THEN      
	 ...
	 UPDATE OPEOWN.TB_OR_OM_SCHD_PLAN SET RKI_PRG_STSC ='03'
                WHERE BAS_YM = P_BASYM 
                ;
                P_LD_CN := SQL%ROWCOUNT;
                ...
                UPDATE OPEOWN.TB_OR_OM_SCHD SET RKI_PRG_STSC ='03'
                WHERE BAS_YM = P_BASYM 
                ;
                P_LD_CN := SQL%ROWCOUNT;
   ...
   USING (
   SELECT BAS_YM,B.RKI_EVL_TGT_YN ,B.RKI_ST_DT,B.RKI_ED_DT,B.RKI_PRG_STSC,B.RKI_ACT_ST_DT,B.RKI_ACT_ED_DT
     FROM OPEOWN.TB_OR_OM_SCHD_PLAN B
   WHERE BAS_YM = 
          (SELECT MIN(BAS_YM) 
           FROM OPEOWN.TB_OR_OM_SCHD_PLAN 
          WHERE BAS_YM  > P_BASYM
            AND RKI_EVL_TGT_YN = 'Y')
            
     ) B
     ON (A.BAS_YM = B.BAS_YM)
     WHEN MATCHED THEN UPDATE SET 
                  A.RKI_ST_DT = B.RKI_ST_DT
                 ,A.RKI_ED_DT = B.RKI_ED_DT
                 ,A.RKI_ACT_ST_DT = B.RKI_ACT_ST_DT
                 ,A.RKI_ACT_ED_DT = B.RKI_ACT_ED_DT
                 ,A.RKI_PRG_STSC = B.RKI_PRG_STSC
                 ,A.RKI_EVL_TGT_YN = B.RKI_EVL_TGT_YN
                 ,A.LSCHG_DTM = SYSDATE
                 ,A.LS_WKR_ENO = 'SYSTEM'
     WHEN NOT MATCHED THEN 
     INSERT (A.GRP_ORG_C,A.BAS_YM
                 ,A.RKI_ST_DT,A.RKI_ED_DT,A.RKI_ACT_ST_DT,A.RKI_ACT_ED_DT
                 ,A.RKI_PRG_STSC,A.RKI_EVL_TGT_YN,A.FIR_INP_DTM,A.FIR_INPMN_ENO,A.LSCHG_DTM,A.LS_WKR_ENO
                 )
           VALUES ('01',B.BAS_YM,B.RKI_ST_DT,B.RKI_ED_DT,B.RKI_ACT_ST_DT,B.RKI_ACT_ED_DT
                 ,B.RKI_PRG_STSC,B.RKI_EVL_TGT_YN,SYSDATE,'SYSTEM',SYSDATE,'SYSTEM'
                 )
             ;
             
           DBMS_OUTPUT.PUT_LINE(P_LD_CN || '다음KRI 스케쥴 생성 평가완료로 업데이트');
	END IF;
