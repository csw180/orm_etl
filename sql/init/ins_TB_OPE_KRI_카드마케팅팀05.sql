/*
  프로그램명 : ins_TB_OPE_KRI_카드마케팅팀05
  타켓테이블 : TB_OPE_KRI_카드마케팅팀05
  협      조 : 박은경대리
  최조작성자 : 최상원
  KRI 지표명 :
     - 카드마케팅팀-05: 장기미교부 신용카드 수
       A: 전월 기준 카드발송일로 부터 1개월간 미교부된 영업점/지점 보관카드수
*/
DECLARE
...
BEGIN
  ...
  IF P_EOTM_DT = P_BASEDAY  THEN
    ...
    DELETE OPEOWN.TB_OPE_KRI_카드마케팅팀05
    WHERE  STD_DT = P_BASEDAY;

    DBMS_OUTPUT.PUT_LINE(SQL%ROWCOUNT || ' rows deleted');

    INSERT INTO OPEOWN.TB_OPE_KRI_카드마케팅팀05
    SELECT
            P_BASEDAY
           ,T.HGOV_BRNO
           ,Y.BR_NM
    ...
    FROM  (
            ...
            WHERE
              ...
              AND  A.BR_ACP_DT         BETWEEN P_SOTM_DT  AND P_EOTM_DT     --점접수일자
            ...
            WHERE  A.CRD_NO = B.CRD_NO
              ...
              AND  A.BR_ACP_DT         BETWEEN P_SOTM_DT  AND P_EOTM_DT
        ) T
       , OT_DWA_DD_BR_BC  Y
    WHERE 1=1
    AND   T.HGOV_BRNO = Y.BRNO
    AND   Y.STD_DT    = P_BASEDAY
    AND   HLDG_CRD_PCS_DSCD = '10'  -- 보유카드처리구분코드(10:접수,20:이관,30:교부,40:발송,50:폐기)
    AND   ( TRIM(T.SHPP_ENVL_DT) IS NOT NULL AND TO_CHAR(ADD_MONTHS(TO_DATE(T.SHPP_ENVL_DT,'YYYYMMDD'),1),'YYYYMMDD') <= P_BASEDAY)
    ...
    SP_INS_ETLLOG('TB_OPE_KRI_카드마케팅팀05',P_BASEDAY,P_LD_CN,'KRI_ETL');
  END IF;
